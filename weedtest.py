...
class image_detector:
    ...
    def movebase_client(self,area,map_coordinate):
        ...
        if area is not None: 
            ...
            print("goal sent at",goal.target_pose.pose)
    ...
    def image_processing(self, data):
        ...
        '''
        ### onion field image processing Hough Trans
        eadges=cv2.Canny(res,150,300)
        # get lines   # (x1, y1, x2, y2)
        lines = cv2.HoughLinesP(edges,rho=1.0,theta=np.pi/180,threshold=20,minLineLength=30,maxLineGap=10)

        line_img = np.zeros((res.shape[0], res.shape[1], 3), dtype=np.uint8)
        line_color = [0, 255, 0]
        line_thickness = 2
        ...
        overlay = cv2.addWeighted(res, 0.8, line_img, 1.0, 0.0)
        cv2.imshow("Overlay HaughLines over res", overlay)
        '''
        # blur mask
    ...
    def callback(self,data):
        ...
        if not self.camera_is_ready:
            ...
            return
        if self.robot_state is not 'idle':
            print('ignoring callback - robot is busy', self.robot_state)
            return

        ### run image processing   ### 
        mask,res,(cx,cy),area = self.image_processing(data)
        if area == None: 
            result=self.movebase_client(area,None)
            #todo: error checking
            return           
        ...
        self.robot_state = 'idle'                             # states whether the robot is idle
        #if area > 2100: # if area >2100
        self.service_client.call()
        self.spray_nr+=1                                       # keep track of weeds sprayed 
        print("sprayed %d weeds"%self.spray_nr)                        
        #else:
        #    return         
